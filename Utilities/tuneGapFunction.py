...
def parseArgs(options, args, p):
    if len(args) != 4:
        p.print_help()
        p.error("There must be three arguments.")
    ...
    while i <= gap_open_function_end[0]:
        ...
        while j <= gap_open_function_end[1]:
            ...
            my_outfile = os.path.join(outdir, fstring_middle, my_output_file_name)
            if not os.path.exists(os.path.join(outdir, fstring_middle)):
                os.makedirs(os.path.join(outdir, fstring_middle))
            scoring_filename = os.path.join(outdir, fstring_middle, "scoring." + fstring + ".txt")
            ...
            p_handle = pool.apply_async(runBisPinCalculate, (scoring_filename, 2, reference_genome_file, my_outfile, reads, number_of_reads, fstring, fstring_middle))
    ...
    for p in processes_get:
        ...
        sys.stderr.write("Finished with work:\t%s\n" % (str(completed_string)))
        sys.stderr.flush()
...
def main():
    ...
    p.add_option('--scoring_file','-s',help='A text file representing the beginning values of the function. [default: %default]', default = "./scoring_function.txt")
    p.add_option('--gap_open_function_begin','-b',help='A string representing the beginning values of the function. [default: %default]', default = "0.05,0.0")
    p.add_option('--gap_open_function_end','-e',help='A string representing the ending values of the function. [default: %default]', default = "1.0,150")
    p.add_option('--gap_increment_function', '-i', help='Step amounts file.', default = "0.05,5")
    p.add_option('--processes', '-p', help = 'The number of processes to execute at once.', default = '6')
    #p.add_option('--edit','-e',help='[default: %default]', action='store_true', default = False)
    #p.add_option()
